...
while True:
    ...
    if landmarks:
        ...
        if int(joint_angle_left_arm) <= 35 and rep_left_arm is False:
            rep_counter_left_arm += 1
            rep_left_arm = True
        elif int(joint_angle_left_arm) <= 35 and rep_left_arm is True:
            rep_left_arm = True
        else:
            rep_left_arm = False

        percent = 100.0 - np.interp(joint_angle_left_arm, (35, 160), (0, 100))
        ...
        # Display Reps
        cv.putText(frame, f'Left Arm Reps: {str(rep_counter_left_arm)}', (10, 80), cv.FONT_HERSHEY_COMPLEX, 1,
                   (0, 255, 0), 2)
